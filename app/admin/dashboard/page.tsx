...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'upload' | 'resources' | 'logs'>('orders');
  
  const [isDarkMode, setIsDarkMode] = useState(false);
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false);

  // New Feature: Realtime Notifications List
  const [notifications, setNotifications] = useState<string[]>([]);
  const [showNotifDropdown, setShowNotifDropdown] = useState(false);

  // New Feature: Customer Modal State
  const [selectedCustomer, setSelectedCustomer] = useState<{ name: string; email: string } | null>(null);

  // New Feature: Analytics Date Range Filter ('all' | '3months' | '6months' | '1year')
  ...
  const storageLimitMB = 1024; // 1 GB limit
  ...
  const calculateStorageSize = useCallback(async () => {
    try {
      ...
      setStorageUsedMB(Number(totalMB.toFixed(2)));
    } catch (err) {
  ...
  // REALTIME SUBSCRIPTION FOR NEW ORDERS & NOTIFICATIONS
  useEffect(() => {
  ...
  const checkAdminSession = useCallback(async () => {
    ...
    setLoading(false);
  }, [router, fetchOrders, fetchResources, fetchLogs, calculateStorageSize]);
  ...
  const handleUpdateOrderStatus = async (orderId: string, newStatus: 'approved' | 'rejected', buyerEmail?: string, resourceTitle?: string) => {
    try {
      ...
      // FEATURE 1: Email Notification Simulation / Trigger
      if (buyerEmail) {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredOrders.map(o => [
      ...
      `"${o.buyer_name}"`, 
      o.buyer_email, 
      `"${o.resources?.title || 'N/A'}"`, 
      o.amount, 
      o.gcash_ref_number || 'N/A', 
      o.status, 
    ...
    logActivity('EXPORT_CSV', `Nag-export ng ${filteredOrders.length} orders patungong CSV file kasama ang Purchase Date`);
  };
  ...
  // FEATURE 5: Download / Sales Count computation per resource
  ...
  // FEATURE 3: Analytics Date Range Filter implementation
  ...
  // FEATURE 4: Customer Purchase History list generator
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col md:flex-row transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-100 text-slate-800'}`}>
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className={`flex flex-col justify-between border-r shrink-0 transition-all duration-300 ${
        isSidebarCollapsed ? 'md:w-20' : 'md:w-64'
      } w-full bg-slate-900 border-slate-800 text-slate-300 h-screen sticky top-0`}>
          ...
          <div className="py-4 overflow-y-auto">
            ...
            <nav className="px-4 space-y-1.5 mb-6">
              <button
                ...
                className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer ${
                  activeTab === 'orders' ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                }`}
              >
                ...
                {!isSidebarCollapsed && pendingCount > 0 && (
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${activeTab === 'orders' ? 'bg-white/20 text-white' : 'bg-amber-500 text-slate-950'}`}>
                    {pendingCount}
                  </span>
                )}
              </button>
            ...
            <nav className="px-4 space-y-1.5 mb-6">
              <button
                ...
                className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-2.5'} px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer ${
                  activeTab === 'upload' ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                }`}
              >
                <span className="text-base">📤</span>
                {!isSidebarCollapsed && <span>Upload Material</span>}
              </button>
              <button
                onClick={() => setActiveTab('resources')}
                className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer ${
                  activeTab === 'resources' ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                }`}
              >
                <span className="flex items-center gap-2.5">
                  <span className="text-base">📚</span>
                  {!isSidebarCollapsed && <span>Resources List</span>}
                </span>
            ...
            <nav className="px-4 space-y-1.5">
              <button
                ...
                className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-2.5'} px-4 py-3 rounded-xl text-xs font-bold transition cursor-pointer ${
                  activeTab === 'logs' ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                }`}
              >
                <span className="text-base">📜</span>
        ...
        <div className="p-4 border-t border-slate-800/80 space-y-4">
          ...
          <button
            ...
            className="w-full bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 font-semibold py-2.5 rounded-xl text-xs transition cursor-pointer flex items-center justify-center gap-2"
          >
            <span>🚪</span>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className={`border-b px-6 py-4 flex justify-between items-center shadow-xs transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
          <div className="flex items-center gap-4">
            <button
              ...
              className={`p-2 rounded-xl transition cursor-pointer ${isDarkMode ? 'bg-slate-800 hover:bg-slate-700 text-slate-300' : 'bg-slate-100 hover:bg-slate-200 text-slate-600'}`}
            >
          ...
          <div className="flex items-center gap-3">
            {/* FEATURE 2: REALTIME NOTIFICATION BELL */}
            <div className="relative">
              <button
                onClick={() => setShowNotifDropdown(!showNotifDropdown)}
                className={`relative p-2 rounded-xl border text-xs font-bold transition cursor-pointer ${
              ...
              {showNotifDropdown && (
                <div className={`absolute right-0 mt-2 w-80 rounded-2xl border shadow-xl z-50 p-3 space-y-2 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-200' : 'bg-white border-slate-200 text-slate-800'}`}>
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-1.5">
                    {notifications.length === 0 ? (
                      <p className="text-[11px] text-slate-400 text-center py-4">Walang bagong notification.</p>
                    ) : (
                      notifications.map((notif, index) => (
                        <div key={index} className="p-2 rounded-xl text-[11px] bg-blue-500/10 text-blue-300 border border-blue-500/20">
                          {notif}
                        </div>
                      ))
                    )}
            ...
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer border ${
                isDarkMode ? 'bg-slate-800 text-amber-400 border-slate-700 hover:bg-slate-700' : 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200'
              }`}
            >
        ...
        <div className="p-6 md:p-8 max-w-6xl mx-auto w-full space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className={`p-5 rounded-2xl border shadow-xs flex flex-col justify-between transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
          ...
          {/* FEATURE 3: ANALYTICS & DATE RANGE FILTER CHART */}
          <div className={`border rounded-2xl p-6 shadow-xs transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-2">
                <select 
                  ...
                  onChange={(e: any) => setAnalyticsFilter(e.target.value)}
                  className={`border rounded-xl px-3 py-1.5 text-xs font-bold cursor-pointer focus:outline-none ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-200' : 'bg-slate-50 border-slate-200 text-slate-700'}`}
                >
            ...
            <div className={`h-48 flex items-end justify-between gap-2 sm:gap-4 pt-6 px-2 border-b ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
              {monthlyRevenueData.map((item, index) => (
                <div key={index} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  <div className={`text-[10px] font-bold opacity-0 group-hover:opacity-100 transition whitespace-nowrap ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
          ...
          {/* TAB 1: MANAGE ORDERS */}
          {activeTab === 'orders' && (
            <div className={`border rounded-2xl shadow-xs overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
                      ...
                      <tbody className={`divide-y ${isDarkMode ? 'divide-slate-800 text-slate-300' : 'divide-slate-100 text-slate-700'}`}>
                        {paginatedOrders.map((order) => (
                          <tr key={order.id} className={`transition ${isDarkMode ? 'hover:bg-slate-800/40' : 'hover:bg-slate-50/50'}`}>
                            ...
                            <td className="p-4">
                              {/* FEATURE 4: Click customer name to open profile history modal */}
                              <button 
                                onClick={() => setSelectedCustomer({ name: order.buyer_name, email: order.buyer_email })}
                                className={`font-bold text-left hover:underline cursor-pointer ${isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}
                              >
                                {order.buyer_name}
                              </button>
          ...
          {/* TAB 3: RESOURCES LIST (With Feature 5: Download/Sales Count) */}
          {activeTab === 'resources' && (
            <div className={`border rounded-2xl shadow-xs overflow-hidden transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200/80'}`}>
              <div className={`p-5 border-b flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${isDarkMode ? 'border-slate-800' : 'border-slate-100'}`}>
      ...
      {/* FEATURE 4: CUSTOMER HISTORY MODAL */}
      {selectedCustomer && (
          ...
          <div className={`border rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200 text-slate-900'}`}>
            <div className="flex justify-between items-center border-b pb-3 border-slate-700/50">
              ...
              <button 
                ...
                className="w-8 h-8 rounded-full bg-slate-800 text-slate-300 flex items-center justify-center font-bold hover:bg-slate-700 cursor-pointer"
              >
                ✕
              </button>
            </div>
